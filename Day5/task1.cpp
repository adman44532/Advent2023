...
int main()
{
   ...
   *//*
   ...
   for (auto& seed : seeds)
   {
      long long ogSeed = seed;
      for (const auto& map : maps)
      {
         mapItem(map, seed); // Constantly alter the seed through all the maps
      }
      //std::cout << "Seed #" << ogSeed << " has been mapped to location: " << seed << "\n";
   }

   // We now have location values, find the lowest value in the vector
   long long lowestValue = seeds[0];
   for (const auto& location : seeds)
   {
   ...
   std::cout << "The lowest location is: " << lowestValue << "\n";
}